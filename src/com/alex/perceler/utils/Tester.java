	...
	public static boolean lengthValide(String str, int index, String newStr)
		...
		if(str.length()>30)
			...
			if(newStr.length()>30)
				...
				//Variables.getDebugError().add(new autoCorrectError(str, index, "\""+str+"\" , sera remplac�e par : \""+newStr+"\"","M�me avec correction la taille est ici sup�rieur � 30 caract�res. Essayez d'y rem�dier",true));
				}
			else
				{
				//Variables.getDebugError().add(new autoCorrectError(str, index, "\""+str+"\" , sera remplac�e par : \""+newStr+"\"","",false));	
				}
			return false;
			}
		else
